...
import { NetService } from "@nyx/shared/Net";
...
export const DEFAULT_NYX_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(homedir(), ".nyx"),
);
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@nyx/contracts",
    "--filter=@nyx/web",
    "--filter=nyx",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=nyx"],
  "dev:web": ["run", "dev", "--filter=@nyx/web"],
  "dev:desktop": ["run", "dev", "--filter=@nyx/desktop", "--filter=@nyx/web", "--parallel"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("NYX_PORT_OFFSET"),
  devInstance: optionalStringConfig("NYX_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid NYX_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `NYX_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric NYX_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed NYX_DEV_INSTANCE=${seed}` };
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly nyxHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(nyxHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      NYX_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.NYX_PORT = String(serverPort);
      output.VITE_WS_URL = `ws://localhost:${serverPort}`;
    } else {
      delete output.NYX_PORT;
      delete output.VITE_WS_URL;
      delete output.NYX_AUTH_TOKEN;
      delete output.NYX_MODE;
      delete output.NYX_NO_BROWSER;
      delete output.NYX_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.NYX_HOST = host;
    }

    if (!isDesktopMode && authToken !== undefined) {
      output.NYX_AUTH_TOKEN = authToken;
    } else if (!isDesktopMode) {
      delete output.NYX_AUTH_TOKEN;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.NYX_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.NYX_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.NYX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.NYX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.NYX_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.NYX_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.NYX_MODE = "web";
      delete output.NYX_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read NYX_PORT_OFFSET/NYX_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("NYX_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv("NYX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD"),
      logWebSocketEvents: readOptionalBooleanEnv("NYX_LOG_WS_EVENTS"),
    };
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.NYX_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.NYX_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  nyxHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all Nyx data (equivalent to NYX_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("NYX_HOME")),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to NYX_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("NYX_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to NYX_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("NYX_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to NYX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("NYX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to NYX_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("NYX_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to NYX_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("NYX_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to NYX_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("NYX_PORT")),
  ),
