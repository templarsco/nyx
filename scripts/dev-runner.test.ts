...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit NYX_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "NYX_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid NYX_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults NYX_HOME to ~/.t3 when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.NYX_HOME, resolve(homedir(), ".nyx"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.NYX_HOME, resolve("/tmp/custom-t3"));
        assert.equal(env.NYX_PORT, "4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.NYX_NO_BROWSER, "1");
        assert.equal(env.NYX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.NYX_LOG_WS_EVENTS, "1");
        assert.equal(env.NYX_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.NYX_MODE, "web");
        assert.equal(env.NYX_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.NYX_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom nyxHome when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.NYX_HOME, resolve("/tmp/my-t3"));
      }),
    );
    ...
    it.effect("does not export backend bootstrap env for dev:desktop", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.NYX_PORT, undefined);
        assert.equal(env.NYX_AUTH_TOKEN, undefined);
        assert.equal(env.NYX_MODE, undefined);
        assert.equal(env.NYX_NO_BROWSER, undefined);
        assert.equal(env.NYX_HOST, undefined);
