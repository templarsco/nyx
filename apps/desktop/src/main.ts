...
import type { ContextMenuItem } from "@nyx/contracts";
import { NetService } from "@nyx/shared/Net";
import { RotatingFileSink } from "@nyx/shared/logging";
import { parsePersistedServerObservabilitySettings } from "@nyx/shared/serverSettings";
...
const BASE_DIR = process.env.NYX_HOME?.trim() || Path.join(OS.homedir(), ".nyx");
const STATE_DIR = Path.join(BASE_DIR, "userdata");
const DESKTOP_SCHEME = "nyx";
const ROOT_DIR = Path.resolve(__dirname, "../../..");
const isDevelopment = Boolean(process.env.VITE_DEV_SERVER_URL);
const APP_DISPLAY_NAME = isDevelopment ? "Nyx (Dev)" : "Nyx";
const APP_USER_MODEL_ID = "com.templarsco.nyx";
const LINUX_DESKTOP_ENTRY_NAME = isDevelopment ? "nyx-dev.desktop" : "nyx.desktop";
const LINUX_WM_CLASS = isDevelopment ? "nyx-dev" : "nyx";
const USER_DATA_DIR_NAME = isDevelopment ? "nyx-dev" : "nyx";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "Nyx (Dev)" : "Nyx";
...
function backendChildEnv(): NodeJS.ProcessEnv {
  ...
  delete env.NYX_PORT;
  delete env.NYX_AUTH_TOKEN;
  delete env.NYX_MODE;
  delete env.NYX_NO_BROWSER;
  delete env.NYX_HOST;
  delete env.NYX_DESKTOP_WS_URL;
  return env;
}
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { nyxCommitHash?: unknown };
    return normalizeCommitHash(parsed.nyxCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.NYX_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Nyx failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const disabledReason = getAutoUpdateDisabledReason({
    ...
    disabledByEnv: process.env.NYX_DISABLE_AUTO_UPDATE === "1",
  });
  if (disabledReason) {
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Nyx ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/Nyx (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`nyx`). If the legacy
...
function configureAutoUpdater(): void {
  ...
  const githubToken =
    process.env.NYX_DESKTOP_UPDATE_GITHUB_TOKEN?.trim() || process.env.GH_TOKEN?.trim() || "";
  if (githubToken) {
  ...
  if (process.env.NYX_DESKTOP_MOCK_UPDATES) {
    autoUpdater.setFeedURL({
      provider: "generic",
      url: `http://localhost:${process.env.NYX_DESKTOP_MOCK_UPDATE_SERVER_PORT ?? 3000}`,
    });
  }
