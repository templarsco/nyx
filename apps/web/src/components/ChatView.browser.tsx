...
import { DEFAULT_CLIENT_SETTINGS } from "@nyx/contracts/settings";
...
function createBaseServerConfig(): ServerConfig {
  return {
    cwd: "/repo/project",
    keybindingsConfigPath: "/repo/project/.nyx-keybindings.json",
    keybindings: [],
    issues: [],
    providers: [
      {
...
describe("ChatView timeline estimator parity (full app)", () => {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            worktreePath: null,
            env: {
              NYX_PROJECT_ROOT: "/repo/project",
            },
          });
          expect(openRequest?.env?.NYX_WORKTREE_PATH).toBeUndefined();
        },
        { timeout: 8_000, interval: 16 },
      );
    } finally {
  ...
  it("falls back to the first installed editor when the stored favorite is unavailable", async () => {
    localStorage.setItem("nyx:last-editor", JSON.stringify("vscodium"));
    setDraftThreadWithoutWorktree();
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              NYX_PROJECT_ROOT: "/repo/project",
              NYX_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
          ...
          expect(dispatchRequest).toMatchObject({
            ...
            bootstrap: {
              ...
              prepareWorktree: {
                ...
                baseBranch: "main",
                branch: expect.stringMatching(/^nyx\/[0-9a-f]{8}$/),
              },
              runSetupScript: true,
            },
          });
  ...
  it("shows the confirm archive action after clicking the archive button", async () => {
    ...
    try {
      ...
      localStorage.removeItem("nyx:client-settings:v1");
      await mounted.cleanup();
    }
  });
