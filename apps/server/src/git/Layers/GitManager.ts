...
import { GitManagerError } from "@nyx/contracts";
import {
  GitManager,
...
import type { GitManagerServiceError } from "@nyx/contracts";
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `nyx/pr-${pullRequest.number}/${suffix}`;
}
...
export const makeGitManager = Effect.fn("makeGitManager")(function* () {
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(tempDir, `nyx-pr-body-${process.pid}-${randomUUID()}.md`);
    yield* fileSystem
