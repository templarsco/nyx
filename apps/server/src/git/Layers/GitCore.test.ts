...
import { GitCommandError } from "@nyx/contracts";
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "nyx/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "nyx/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "nyx/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "nyx/tmp-working",
          newBranch: "nyx/feat/session",
        });
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "nyx/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "nyx/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "nyx/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "nyx/tmp-working" });
        ...
        expect(renamed.branch).toBe("nyx/feat/session-2");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* git(tmp, ["ls-remote", "--heads", "jasonLaster", "nyx/pr-488/statemachine"]),
          ).toBe("");
        }),
    );
